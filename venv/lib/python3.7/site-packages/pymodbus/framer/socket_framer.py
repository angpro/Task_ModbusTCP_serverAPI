...
#IP_client_mean =  pymodbus.server.sync.IP_client
#from pymodbus.server.sync import IP_client
#from pymodbus.server.sync import port_client
...
path = '/api/journal/event/'
config_file = "/Users/Makbukuska/Desktop/Task_ModbusTCP/config.cfg"
if len(sys.argv) > 1:
...
class ModbusSocketFramer(ModbusFramer):
    ...
    def processIncomingPacket(self, data, callback, unit, **kwargs):
        ...
        #_logger.info("Processing: " + hexlify_packets(data))
    ...
    def _process(self, callback, error=False):
        ...
        #print (dir(result))
        #print('xxxx-xx-xx xx:xx:xx  '+'address' + '   ' + 'byte_count' + '   ' + 'check'  + '   ' 'function_code'  + '   ' + 'protocol_id' + '   ' + 'skip_encode' + '   ' + 'transaction_id' + '   ' + 'unit_id' + '   ' + 'values')
        _logger.info('   ' +str(result.address)+ '   ' +str(result.byte_count)+'   ' +str(result.check)+ '   ' +str(result.function_code)+ '   '+str(result.protocol_id)+ '   '+str(result.skip_encode)+ '   '+str(result.transaction_id)+ '   '+str(result.unit_id)+ '   '+str(result.values) )
        print('xxxx-xx-xx xx:xx:xx,xxx' + '  '+ 'address' + '   ' + 'byte_count'  + '   ' + 'check' + '   ' + 'function_code' + '   ' + 'protocol_id' + '   ' + 'skip_encode' + '   ' + 'transaction_id' + '   ' + 'unit_id' + '   ' + 'values')
        tr_data = "address: " + str(result.address) + "; "+ "byte_count: "+str(result.byte_count)+"; "+"check: "+str(result.check)+"; "+"function_code: "+str(result.function_code)+"; "+"protocol_id: "+str(result.protocol_id)+"; "+ "skip_encode: "+str(result.skip_encode)+"; "+"unit_id: "+"; "+str(result.unit_id)+"; "+ "values: "+str(result.values)
        mes = {
            "date_time": datetime.datetime.now().strftime("%FT%TZ"),
            "source": str(self.client[0]),
            "destination": str(IP_server),
            "status": "new",
            "comment": "task has been done",
            "country": "LO",
            "device": 3,
            "port_dst": 5020,
            "port_src": str(self.client[1]),
            "type_event": "modbus",
            "msg": tr_data
        }

        data = json.dumps(mes)
        api_req.server_request(path, scheme=None, address=None, port=None, data=data, put=False, eid=None, method=None)
